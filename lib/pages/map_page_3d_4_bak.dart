...
class _MapPageState extends State<MapPage> {
  ...
  // ============================================
  // 수정 2: 새로운 마커 클릭 핸들러 (좌표 기반)
  // ============================================
  Future<void> _handleMarkerClick(LatLng clickedLatLng, LocationsProvider provider) async {
    debugPrint('');
    debugPrint('🎯 ======================================================');
    debugPrint('🎯 [마커 클릭 처리 시작]');
    debugPrint('🎯 좌표: (${clickedLatLng.latitude.toStringAsFixed(7)}, ${clickedLatLng.longitude.toStringAsFixed(7)})');
    ...
    for (int i = 0; i < clusters.length; i++) {
      ...
      if (cluster.length > 1) {
        ...
        // 클릭한 좌표와 클러스터 중심이 거의 일치하는지 확인 (1cm 이내)
        final distanceDegrees = sqrt(
          pow(centerLat - clickedLatLng.latitude, 2) + 
          pow(centerLng - clickedLatLng.longitude, 2)
        );
        ...
        debugPrint('   클러스터 #$i (${cluster.length}명): ${distanceMeters.toStringAsFixed(2)}m');
        
        if (distanceMeters < 1.0) { // 1m 이내면 클러스터로 간주
          debugPrint('   🎯 클러스터 클릭!');
    ...
    for (final loc in locs) {
      ...
      debugPrint('   ${loc.userId.substring(0, 8)}: ${distanceMeters.toStringAsFixed(2)}m');
      
      if (distanceMeters < 1.0) { // 1m 이내면 해당 유저
        debugPrint('   🎯 유저 클릭! (${loc.userId.substring(0, 8)})');
        _showUserInfo(loc);
        return;
      }
    }

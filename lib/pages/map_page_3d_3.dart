...
class _MapPageState extends State<MapPage> {
  ...
  Future<void> _updateMapLibreMarkers(
    ...
    try {
      ...
      if (allLocs.isEmpty) {
        ...
        debugPrint('   provider.locations: ${provider.locations.keys.toList()}');
      ...
      if (locs.isEmpty) {
        ...
        debugPrint('   선택된 그룹: $_selectedGroupName');
        _isUpdatingMarkers = false;
        return;
      }
      ...
      for (final loc in locs) {
        debugPrint('   - ${loc.userId}: (${loc.lat.toStringAsFixed(6)}, ${loc.lng.toStringAsFixed(6)})');
      }
  ...
  // ============================================
  // 수정 2: MapLibre 빌드 - GestureDetector로 감싸기
  // ============================================
  Widget _buildMapLibreMap(LocationsProvider provider) {
    return Stack(
      children: [
        // ✅ GestureDetector로 감싸서 클릭 감지
        GestureDetector(
          onTapUp: (TapUpDetails details) async {
            debugPrint('');
            debugPrint('👆 ========================================');
            debugPrint('👆 [GestureDetector 탭 감지!]');
            debugPrint('👆 화면 좌표: (${details.localPosition.dx}, ${details.localPosition.dy})');
            ...
            // 화면 좌표를 지도 좌표로 변환
            try {
              // MapLibre에는 좌표 변환 API가 없으므로 근사값 계산
              // 현재 카메라 위치와 줌 레벨 사용
              final screenSize = MediaQuery.of(context).size;
              final centerLat = 37.408915; // 초기 중심 (실제로는 카메라 위치 사용)
              final centerLng = 127.148245;
              
              // 간단한 근사 (정확하지 않지만 테스트용)
              final dx = details.localPosition.dx - screenSize.width / 2;
              final dy = details.localPosition.dy - screenSize.height / 2;
              
              // 줌 레벨에 따른 스케일
              final scale = pow(2, 20 - _currentZoom).toDouble();
              final latOffset = -dy * scale / 40000000;
              final lngOffset = dx * scale / 40000000;
              
              final clickedLat = centerLat + latOffset;
              final clickedLng = centerLng + lngOffset;
              
              final clickedLatLng = LatLng(clickedLat, clickedLng);
              debugPrint('👆 추정 지도 좌표: ($clickedLat, $clickedLng)');
              debugPrint('👆 ========================================');
              
              await _handleMapLibreClick(clickedLatLng, provider);
            } catch (e) {
          ...
          child: MapLibreMap(
            ...
            onMapClick: (point, latLng) async {
              // ✅ 혹시 작동한다면 이것도 사용
              debugPrint('🖱️  ✨ onMapClick 작동! (예상 밖)');
              await _handleMapLibreClick(latLng, provider);
            },
  ...
  // 수정 3: 클릭 감지 - 반경 축소
  // ============================================
  Future<void> _handleMapLibreClick(LatLng clickedLatLng, LocationsProvider provider) async {
    debugPrint('');
    debugPrint('🖱️  ======================================================');
    debugPrint('🖱️  [맵 클릭 처리 시작]');
    debugPrint('🖱️  좌표: (${clickedLatLng.latitude.toStringAsFixed(7)}, ${clickedLatLng.longitude.toStringAsFixed(7)})');
    debugPrint('🖱️  줌: ${_currentZoom.toStringAsFixed(2)}');
    ...
    debugPrint('📏 클릭 반경: 클러스터=${clusterRadiusMeters}m, 유저=${userRadiusMeters}m');

    // 1단계: 클러스터 체크
    debugPrint('🔍 [1단계] 클러스터 검색');
    final clusters = _clusterLocations(locs);
    ...
    for (int i = 0; i < clusters.length; i++) {
      ...
      debugPrint('   클러스터 #$i (${cluster.length}명): ${distanceMeters.toStringAsFixed(1)}m');
    ...
    for (final loc in locs) {
      ...
      debugPrint('   ${loc.userId.substring(0, 8)}: ${distanceMeters.toStringAsFixed(1)}m');
      
      if (distanceMeters <= userRadiusMeters && distanceMeters < minDistance) {
    ...
    if (closestUser != null) {
      debugPrint('   🎯 유저 클릭! (${closestUser.userId.substring(0, 8)})');
